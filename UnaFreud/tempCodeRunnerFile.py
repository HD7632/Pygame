...
superficie_texto = fuente.render("¡Perdiste Caremonda!", True, (255, 255, 255))
...
while runningcode:
    ...
    if state == 'play':
        ...
        if player.hitbox.colliderect(truck.ammo1_hitbox):
            player.damage(10)
            player.inatack = True
        ...
        #Actualizo al camiond
        truck.update_truck(dt)
    ...
    if state == 'defeat':
        ...
        object.screen.blit(superficie_texto, (76, 56))
        #Deteccion de teclas y de mouse
        for event in pygame.event.get():
            #Programo que el juego se cierre cuando el usuario haga clic en la x de la ventana
            if event.type == pygame.QUIT:
